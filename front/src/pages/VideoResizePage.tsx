...
const VideoResizePage = () => {
  ...
  const handleMouseMove = (event) => {
    if (dragging) {
      const newWidth = event.clientX - canvasRef.current.getBoundingClientRect().left;
      const newHeight = event.clientY - canvasRef.current.getBoundingClientRect().top;
  ...
  const resizeVideo = async () => {
    ...
    await ffmpeg.run('-i', inputFilename, '-vf', `scale=${videoSize.width}:${videoSize.height}`, outputFilename);
  ...
  return (
    <div>
      ...
      <canvas ref={canvasRef} onMouseDown={handleMouseDown} onMouseUp={handleMouseUp} onMouseMove={handleMouseMove}></canvas>
      {outputVideo && (
        <div>
          <h3>Resized Video</h3>
      ...
      <button onClick={resizeVideo} disabled={processing}>Resize Video</button>
    </div>
  );
};
